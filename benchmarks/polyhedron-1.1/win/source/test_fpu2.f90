...
! RJA Turn dimensions into parameters
INTEGER, PARAMETER :: smallsize=250, smallits=1000 ! RJA initial values were 101 and 1000.
INTEGER, PARAMETER :: bigsize=2000  ! RJA initial value was 1000


! - - - local variables - - -
REAL(RK8) :: pool(smallsize,smallsize,smallits), pool3(bigsize,bigsize) ! random numbers to invert
! EQUIVALENCE (pool,pool3)               ! use same pool numbers for test 3,4
REAL(RK8) :: a(smallsize,smallsize), a3(bigsize,bigsize)     ! working matrices
...
DO n = 1,4
   ...
   CASE (1)
      DO i = 1,smallits
         ...
         CALL Gauss (a,smallsize)  ! invert a
         CALL Gauss (a,smallsize)  ! invert a
      END DO
      ...
      WRITE (*,42)"Test1: Gauss  - ", smallits," (",smallsize,"x",smallsize,") inverts in ", dt(n), ' seconds  Err=', avg_err
 
   CASE (2)
      DO i = 1,smallits
         a = pool(:,:,i)         ! get next matrix to invert
         CALL Crout (a,smallsize)  ! invert a
         CALL Crout (a,smallsize)  ! invert a
      END DO
      ...
      CALL SYSTEM_CLOCK (clock2,rate)
      dt(n) = (clock2-clock1)/DBLE(rate)  ! get benchmark (n) elapsed sec.   
      WRITE (*,42)"Test2: Crout  - ", smallits," (",smallsize,"x",smallsize,") inverts in ", dt(n), ' seconds  Err=', avg_err

   CASE (3)
      a3 = pool3                 ! get 1001x1001 matrix
      CALL Crout (a3,bigsize)     ! invert a3
      CALL Crout (a3,bigsize)     ! invert a3
      
      avg_err = SUM(ABS(a3-pool3))/(bigsize*bigsize)    ! invert err.
      CALL SYSTEM_CLOCK (clock2,rate)
      dt(n) = (clock2-clock1)/DBLE(rate)  ! get benchmark (n) elapsed sec.   
      WRITE (*,42)"Test3: Crout  - ", 2," (",bigsize,"x",bigsize,") inverts in ", dt(n), ' seconds  Err=', avg_err

   CASE (4)
      a3 = pool3                 ! get 1001x1001 matrix
      CALL Lapack (a3,bigsize)    ! invert a3
      CALL Lapack (a3,bigsize)    ! invert a3
      
      avg_err = SUM(ABS(a3-pool3))/(bigsize*bigsize)    ! invert err.
      CALL SYSTEM_CLOCK (clock2,rate)
      dt(n) = (clock2-clock1)/DBLE(rate)  ! get benchmark (n) elapsed sec.   
      WRITE (*,42)"Test4: Lapack - ", 2," (",bigsize,"x",bigsize,") inverts in ", dt(n), ' seconds  Err=', avg_err
...
42 FORMAT (A,I4,A,I4,A,I4,A,F7.3,A,F14.12)
