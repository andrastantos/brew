...
void SetThreadAffinity(std::thread &aThread, size_t aCpuId) {
	SIM_ASSERT(aCpuId < std::thread::hardware_concurrency());
	SIM_ASSERT(aCpuId < (sizeof(DWORD_PTR) *8));
	DWORD_PTR CpuMask = DWORD_PTR(1) << aCpuId;
	DWORD_PTR RetVal = SetThreadAffinityMask((void*)aThread.native_handle(), CpuMask);
