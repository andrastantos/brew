...
#ifndef NO_ASSERTS
#define SIM_ASSERT(aCond) do { if (!(aCond)) { std::stringstream ErrorStr; ErrorStr << "SIM_ASSERT at " << __FILE__ << ":" << DecPrinter(__LINE__,0) << "(" << #aCond << " failed)"; std::cout << ErrorStr.str() << std::endl; throw Assert_x(ErrorStr.str()); }} while (false);
#else
#define SIM_ASSERT(aCond)
#endif

#ifndef NO_ASSERTS
#define SIM_FIXME do { std::stringstream ErrorStr; ErrorStr << "SIM_FIXME at " << __FILE__ << ":" << DecPrinter(__LINE__,0) << "(" << #aCond << " failed)"; std::cout << ErrorStr.str() << std::endl; throw Assert_x(ErrorStr.str()); } while (false);
#else
#define SIM_FIXME
#endif
